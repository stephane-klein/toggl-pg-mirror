...
if [ -n "$POSTGRES_CONTAINER_ID" ]; then
  ...
  export TOGGL_PG_MIRROR_MCP_READER_URL="postgres://${TOGGL_PG_MIRROR_MCP_READER_POSTGRES_ROLE}:${TOGGL_PG_MIRROR_MCP_READER_POSTGRES_PASSWORD}@${POSTGRES_HOST}:${POSTGRES_PORT}/${POSTGRES_DB}"
else
  unset TOGGL_PG_MIRROR_POSTGRES_URL
  unset TOGGL_PG_MIRROR_MCP_READER_URL
fi
